...
import argparse,hashlib,html,json,math,random,shutil,tempfile,urllib.parse,urllib.request
from pathlib import Path
from PIL import Image,ImageOps
API="https://commons.wikimedia.org/w/api.php";UA="AerialTrack-ReuseFirst/1.0 (commercial provenance collector)";SEED=1337;Q=94
...
def api(p):
 q=urllib.parse.urlencode({**p,"format":"json","formatversion":"2"});r=urllib.request.Request(f"{API}?{q}",headers={"User-Agent":UA})
 with urllib.request.urlopen(r,timeout=60) as x:return json.load(x)
def rows(cat,limit):
 out=[];cont={}
 while len(out)<limit:
  pay=api({"action":"query","generator":"categorymembers","gcmtitle":cat,"gcmtype":"file","gcmlimit":str(min(100,max(1,limit-len(out)))),"prop":"imageinfo","iiprop":"url|extmetadata|sha1|mime|size","iiurlwidth":"1280",**cont})
  ...
  c=pay.get("continue")
  if not c:break
  cont={k:str(v) for k,v in c.items() if k!="continue"}
 return out
def sha(p):
 h=hashlib.sha256()
 with p.open("rb") as f:
...
def dl(url,p):
 ...
 with urllib.request.urlopen(r,timeout=90) as x,p.open("wb") as f:
...
def main():
 ...
 cand=[]
 for c in CATS:cand+=rows(c,max(need*3,120))
 unique={};seen=set()
 for r in cand:
 ...
 for split,n in (("train",a.train),("validation",a.validation),("test",a.test)):
  ...
  while got<n and cur<len(rr):
   ...
   except Exception as e:print("SKIP",r["source_page"],e);continue
   m["items"]+=v;added+=v;existing.add(g);got+=1
 ...
 m["licenses_present"]=sorted({str(i.get("license")) for i in m["items"] if i.get("license")});a.manifest.write_text(json.dumps(m,indent=2)+"\n");print("AERIALTRACK_BALLOON_CONFOUNDERS_APPENDED",len(added));print("AERIALTRACK_BALLOON_HELDOUT",len(test),len(groups))
