...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--corpus',required=True,type=Path);ap.add_argument('--data-root',required=True,type=Path);ap.add_argument('--candidate-state',required=True,type=Path);ap.add_argument('--out-dir',required=True,type=Path);a=ap.parse_args();corp=json.loads(a.corpus.read_text());payload=torch.load(a.candidate_state,map_location='cpu',weights_only=False);model=multi.build().eval();model.load_state_dict(payload['state_dict'],strict=True);val=[r for r in corp['items'] if r['split']=='validation'];test=[r for r in corp['items'] if r['split']=='test'];vm,vp,vy,vids=run_split(model,val,a.data_root);t=choose(vm,vy);vmet=metrics(vm,vy,t);print('AERIALTRACK_PAIRWISE_VALIDATION',json.dumps({'threshold':t,**vmet},sort_keys=True),flush=True);a.out_dir.mkdir(parents=True,exist_ok=True);frozen=a.out_dir/'pairwise_candidate_state.pt';new=dict(payload);new['architecture']='torchvision_mobilenet_v3_small_5way_pairwise_collapse';new['production_scoring_contract']='binary_softmax(drone_logit,max(non_drone_logits))';new['frozen_drone_margin_threshold']=t;new['original_candidate_state_sha256']=sha(a.candidate_state);new['validation_ids_sha256']=hashlib.sha256('\n'.join(vids).encode()).hexdigest();new['final_test_used_for_checkpoint_or_threshold_selection']=False;new['selection_process_loaded_final_test_media']=False;torch.save(new,frozen);selection={'schema_version':3,'candidate_id':'mobilenetv3_small_5way_pairwise_collapse','architecture':new['architecture'],'production_scoring_contract':new['production_scoring_contract'],'pretrained_weights':None,'corpus_manifest_sha256':sha(a.corpus),'candidate_state_sha256':sha(frozen),'original_candidate_state_sha256':sha(a.candidate_state),'checkpoint_selected_on_validation_only':True,'threshold_frozen_before_final_eval':True,'drone_margin_threshold':t,'validation_metrics':vmet,'validation_gate_pass':vmet['drone_precision']>=.90 and vmet['drone_recall']>=.80 and vmet['negative_specificity']>=.95,'final_test_used_for_checkpoint_or_threshold_selection':False,'selection_process_loaded_final_test_media':False};(a.out_dir/'selection_report.json').write_text(json.dumps(selection,indent=2)+'\n')
 tm,tp,ty,tids=run_split(model,test,a.data_root);report=test_report(test,tm,ty,t);report.update({'schema_version':5,'candidate_id':selection['candidate_id'],'threshold':t,'score_kind':'pairwise_drone_margin','threshold_frozen_before_eval':True,'checkpoint_selected_on_validation_only':True,'final_test_used_for_checkpoint_or_threshold_selection':False,'selection_process_loaded_final_test_media':False,'final_test_local_files_verified_after_freeze':True,'corpus_manifest_sha256':sha(a.corpus),'candidate_state_sha256':sha(frozen),'validation_gate_pass':selection['validation_gate_pass']});pred={'candidate_id':selection['candidate_id'],'threshold':t,'score_kind':'pairwise_drone_margin','threshold_frozen_before_eval':True,'predictions':[{'id':i,'drone_margin':float(s),'prob_drone':float(p)} for i,s,p in zip(tids,tm,tp)]};(a.out_dir/'final_test_benchmark.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n');(a.out_dir/'final_test_predictions.json').write_text(json.dumps(pred,indent=2)+'\n');print(json.dumps(report,indent=2,sort_keys=True));print('AERIALTRACK_PAIRWISE_FINAL', 'PASS' if report['quality_pass'] and selection['validation_gate_pass'] else 'FAIL');raise SystemExit(0 if report['quality_pass'] and selection['validation_gate_pass'] else 2)
