...
async function initBackend(){
  try{
    ...
    setMessage('No Firebase configuration is present, so the prototype is running safely in mock mode.');
  }
...
function renderResponders(data){
  ...
  if(mine){const state=freshnessState(now,Number(mine.server_time_ms||mine.last_seen_ms||0),mine.online!==false);myStatusEl.textContent=sharing?state.label:'OFFLINE';}
}
...
async function startSharing(){
  ...
  sharing=true;startBtn.disabled=true;stopBtn.disabled=false;callsignEl.disabled=true;myStatusEl.textContent='STARTING';
